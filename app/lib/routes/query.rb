    ...
    def self.registered(app)
          ...
          adminui_show_table(
            ...
            aux_tables: [
              ...
              UC3Query::QueryClient.client.query(
                ...
                sqlsym: :files_sql,
                dispcols: %w[number source pathname full_size created nodelist unverified actions]
              )
            ]
          )
        end
      ...
      app.post '/queries-update/storage-nodes/add' do
        ...
        if res.fetch(:status, 'FAIL') == 'OK'
          UC3Query::QueryClient.client.query_update('/queries-update/storage-nodes/trigger-replication',
            request.params)
        end
        redirect request.referrer
      end
