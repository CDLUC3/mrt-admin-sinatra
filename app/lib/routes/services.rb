  ...
  module UC3ServicesRoutes
    ...
    def self.registered(app)
      ...
      app.post '/test/purge/*' do |mnemonic|
        ...
        urlparams = {}
        urlparams['count'] = [250, request.params.fetch('count', '50').to_i].min
        ...
        rows.each do |row|
          ...
          if row[:versioned]
            cmd += "\n\naws s3api list-object-versions --bucket #{row[:bucket]}\n" + 
              "  --prefix queue --query 'Versions[?IsLatest==`false`].[Key]' --output text"
          end
    ...
    def service_urls(service)
      ...
      logger.error("Error finding URLs for #{service} instances: #{e}")
      []
    end
    ...
    def load_test_file_to_merritt(file, type, mnemonic)
      logger.info("Loading #{file} to #{mnemonic} #{ui_host}/object/update")
    ...
    def post_url_multipart(url, params)
      logger.info("Multipart POST #{url} with #{params.inspect}")
      uri = URI.parse(url)
