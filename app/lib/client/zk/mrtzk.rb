...
module UC3Queue
  ...
  class ZKClient < UC3::UC3Client
    ...
    def initialize
      ...
      logger.error("ZooKeeper Creation Error: #{e.message}")
        ...
        zk.children('/batches').sort.each do |bid|
          ...
          zk.children("/batches/#{bid}/states/batch-processing").sort.each do |jid|
            if zk.exists?("/jobs/#{jid}/space_needed")
              metrics[:gb_in_process] += zk.get("/jobs/#{jid}/space_needed")[0].to_i / 10_000_000 / 100.0
            end
          end
        end
    ...
    def batches(route)
      ...
      batches.each do |batch|
        ...
        unless batch[:submissionDate].empty? || batch[:status] == 'FAIL'
          logger.info("Batch #{id} found, flagging for age")
    ...
    def cleanup_access_queue
      ZK.open(@zkconn, timeout: 2) do |zk|
        logger.info('Cleaning up access jobs')
        jobs = MerrittZK::Access.list_jobs_as_json(zk)
        jobs = [] if jobs.nil?
        jobs.each do |job|
    ...
    def create_node(path, data: nil)
      ...
      logger.error("Error creating node #{path}: #{e.message}")
    end
    ...
    def lock_collection(mnemonic)
      ...
      logger.error("Error locking collection #{mnemonic}: #{e.message}")
    end
    ...
    def unlock_collection(mnemonic)
      ...
      logger.error("Error unlocking collection #{mnemonic}: #{e.message}")
    end
    ...
    def locked_collections
      ...
      logger.error("Error listing locked collections: #{e.message}")
    end
  end
end
